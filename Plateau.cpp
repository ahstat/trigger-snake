...
Plateau::Plateau(QWidget *parent) : QWidget(parent)
{
 ...
 m_positionEnCoursX=m_tailleTete*Alea::entier_unif_0_n_1(m_grilleLongueur);
 m_positionEnCoursY=m_tailleTete*Alea::entier_unif_0_n_1(m_grilleHauteur); //position init aléatoire
...
void Plateau::paintTout(QPainter &painter) 
{
 //setPalette(QPalette(QColor(250, 250, 200)));
...
void Plateau::paintDensite(QPainter &painter) 
{
 painter.setPen(Qt::NoPen);
...
void Plateau::paintArete(QPainter &painter) 
{ 
 painter.setPen(Qt::NoPen);
...
void Plateau::paintNonOriente(QPainter &painter) 
{
 painter.setPen(Qt::NoPen);
...
void Plateau::paintPomme(QPainter &painter)
{
 painter.setPen(Qt::NoPen);
...
void Plateau::paintShot(QPainter &painter) 
{
 painter.setPen(Qt::NoPen);
...
QRect Plateau::m_rectTete(bool affiche) 
{
...
QPolygon Plateau::m_triangleNord(int x, int y)
{
...
QRect Plateau::m_rectNord(int x, int y)
{
...
QPolygon Plateau::m_triangleSud(int x, int y)
{
 QPolygon triangleN(3);
...
QRect Plateau::m_rectSud(int x, int y)
{
 //int largeur(m_tailleTete/4);
 int largeur(5);
 if(y>0)
...
QPolygon Plateau::m_triangleEst(int x, int y)
{
 QPolygon triangleN(3);
 triangleN.setPoint(0, x*m_tailleTete, y*m_tailleTete);
...
QRect Plateau::m_rectEst(int x, int y) 
{
 //int largeur(m_tailleTete/4);
 int largeur(5);
 if(x>0)
...
QPolygon Plateau::m_triangleOuest(int x, int y)
{
 QPolygon triangleN(3);
...
QRect Plateau::m_rectOuest(int x, int y)
{
 //int largeur(m_tailleTete/4);
 int largeur(5);
...
QRect Plateau::m_rect(int x, int y)
{
...
QRect Plateau::m_rectTeteBord(bool affiche) 
{
 QRect result;
...
QRect Plateau::m_rectCorps(unsigned int i, bool affiche)
{
...
QRect Plateau::m_rectQueue(bool affiche)
{
...
void Plateau::afficher(QRect result) const
{
 std::cout << "(" << result.x() << "," << result.y() << ")"<< std::endl; 
}

void Plateau::updateMove()
{
...
void Plateau::agrandissementSnake()
{
 if(m_nbPasAvantAgrandissementSnake.size() != 0)
 {
  ...
  for(unsigned int i(0); i<m_nbPasAvantAgrandissementSnake.size(); i++)
  {
   ...
   {
    std::cout << "ERREUR Plateau::agrandissementSnake() deux augmentations de longueur en même temps." << std::endl;
   }
  }
  //std::cout << std::endl;
...
void Plateau::decroitPointEnCours()
{
...
bool Plateau::collisionAvecPomme()
{
...
void Plateau::sauvegardePositions()
{
...
void Plateau::sauvegardeTempsPointsNbPommesMangeesLongueurSnake()
{
...
void Plateau::sauvegardePositionsModulo()
{
...
std::string Plateau::convertirDirection(int direction)
{
 if(direction==0)
 {
  return "est";
...
int Plateau::base(int nombre, int base)
{
...
void Plateau::augmenterLongueur()
{
...
void Plateau::nouvellePositionPomme()
{
 ...
 while(X==-1)
 {
  int nouveauX(Alea::entier_unif_0_n_1(m_grilleLongueur));
  int nouveauY(Alea::entier_unif_0_n_1(m_grilleHauteur));
...
bool Plateau::pasDeCollisionAvecSerpent(int nouveauX, int nouveauY)
{
...
int Plateau::mesureDistancePommeTete()
{
